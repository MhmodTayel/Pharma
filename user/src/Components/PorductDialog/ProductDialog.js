          ...
          <div className="row pb-3">
            ...
            <div className="col-md-7">
              <div className="dialogContent">
                ...
                <h6 className="border-bottom pb-3 text-muted text-capitalize">
                  From: {props.medItem.companyProvider}{" "}
                </h6>
                <h5 className={styles.price}>
                  {" "}
                  EPG {props.medItem.pharmPrice}
                </h5>
                ...
                <div className="py-3 mb-0">
                  ...
                  <div>
                    ...
                    <span className={styles.description}>
                      {props.medItem.quantity}
                    </span>
                  </div>
                  <div>
                    <span className={styles.property}>Concentration : </span>{" "}
                    <span className={styles.description}>
                      {props.medItem.concentration}
                    </span>
                  </div>
                  <div>
                    <span className={styles.property}>Categories : </span>
                    {props.medItem?.categories?.map((cat, indx) => (
                      <span key={indx} className={styles.description}>
                        {cat}
                      </span>
                    ))}
                  </div>
                ...
                <Button
                  disabled={orderStore.some(
                    (medicine) => medicine.id == props.medItem.id
                  )}
                  variant="contained"
                  onClick={handleCart}
                  className={styles.cardBtn}
                >
                  <ShoppingCartIcon sx={{ mx: 1, fontSize: 20 }} /> Add to cart
                </Button>
              </div>
            </div>
          </div>
